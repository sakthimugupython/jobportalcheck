...
def applicant_details_view(request, id):
    ...
    applicant_record = Applicant.objects.filter(user=id).first()
...
def update_applicant_status_view(request, applicant_id):
    ...
    if applicant.job.user != request.user:
        messages.error(request, 'You do not have permission to update this applicant status.')
        return redirect('jobapp:dashboard')
    
    form = ApplicantStatusForm(request.POST or None, instance=applicant)
    ...
    return render(request, 'jobapp/update-applicant-status.html', context)


@login_required(login_url=reverse_lazy('account:login'))
@user_is_employer
def schedule_interview_view(request, applicant_id):
    """
    ...
    # Verify employer owns the job
    if applicant.job.user != request.user:
        messages.error(request, 'You do not have permission to schedule interview for this applicant.')
        return redirect('jobapp:dashboard')
    
    # Check if applicant is shortlisted
    if applicant.status != 'shortlisted':
        messages.error(request, 'Only shortlisted applicants can have interviews scheduled.')
    ...
    return render(request, 'jobapp/schedule-interview.html', context)


@login_required(login_url=reverse_lazy('account:login'))
def view_interview_view(request):
    """
    View scheduled interviews for employees
    """
    if request.user.role != 'employee':
        messages.error(request, 'Only employees can view interviews.')
        return redirect('jobapp:dashboard')
    
    interviews = Interview.objects.filter(applicant__user=request.user).select_related('applicant__job')
